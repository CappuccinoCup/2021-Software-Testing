<template>
  <v-container>
    <v-card :color="app.whiteOpacity">
      ...
      <v-card-text>
        <p>
          <span class="mr-4"><b>借据号：</b>{{ this.loan.iouNum }}</span>
          <span class="mr-4"><b>姓名：</b>{{ this.loan.customer.name }}</span>
          <span class="mr-4"><b>性别：</b>{{ this.loan.customer.sex === 0 ? "男" : "女" }}</span>
          <span class="mr-4"><b>客户号：</b>{{ this.loan.customer.code }}</span>
          <span class="mr-4"><b>贷款产品：</b>{{ this.loan.productName }}</span>
          <span class="mr-4"><b>贷款产品编号：</b>{{ this.loan.productName }}</span>
        </p>
        <v-data-table class="elevation-3" :headers="headers" :items="bills" :loading="prodsLoading" disable-sort
          ...
          <template v-slot:item.actions="{ item }">
                          ...
                          <v-row>
                            <v-col cols="6">
                              <v-text-field v-model="accountNum" label="银行账号" append-icon="mdi-account" dense
                                            ...
                                            :rules="[() => !!accountNum || 'username is required']"></v-text-field>
                            </v-col>
                            <v-col cols="6">
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{on, attrs}">
                <v-icon v-bind="attrs" v-on="on" @click="repayAllDialog=true" class="ml-3">
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: '本金', align: 'center', value: 'planAmount'},
        {text: '利息', align: 'center', value: 'planInterest'},
